...
class Selfish_Mining:
    ...
    def __init__(self, **d):
        ...
        self.__display = d.get('display', True)
        self.avgreward1=0
        self.SMreward=12.5
        self.smcount=1
        self.deltasm=0
    ...
    def Simulate(self):
        ...
        print("the sepblock is",SepBlocksEach2016)
        for i in range(0, len(SepBlocksEach2016)):
            ...
            print("\n times blocks for SM",TimesBlocksFoundSM)
            ...
            for ((currentTimestamp, who), block_number) in TimesAllBLocks:
                ...
                if who == 'SM':
                    ...
                    self.smcount+=1
                    self.SMreward+=1
                    self.avgreward1=self.SMreward/self.smcount
                    self.deltasm=math.sqrt(3/2 * math.log(i + 1) / self.smcount)
                    self.ucbsm=self.avgreward1+self.deltasm
                    #print("the ucb for SM is ",self.ucbsm)
                    self.ucbsmcount+=self.ucbsm
                    #till here
                else:
                    ...
                    self.hmcount+=1
                    self.hmreward+=1
                    self.avgreward2=self.hmreward/self.hmcount
                    self.deltahm=math.sqrt(3/2 * math.log(i + 1) / self.hmcount)
                    self.ucbhm=self.avgreward2+self.deltahm
                    #print("the ucb for hM is ",self.ucbhm)
                    self.ucbhmcount+=self.ucbhm
                    #till here  
        ...
        if self.__display:
            print(self)
        print("the ucbcount for SM is ",self.ucbsmcount)
        print("the ucb for HM is ",self.ucbhmcount)
    ...
    def __str__(self):
        ...
        c=self.__counter
        #print(" the no of simulations is ",c)
...
if len(sys.argv)==1:
    ...
    for alpha in alphas:
        for gamma in gammas:
            ...
            new = Selfish_Mining(**{'nb_simulations':100, 'alpha':alpha, 'gamma':gamma, 'write':True})
            new.Simulate()
        count += 1/len(alphas)
